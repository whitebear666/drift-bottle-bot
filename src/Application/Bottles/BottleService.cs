...
public sealed class BottleService
{
    ...
    public async Task<(Guid bottleId, string bottleNo)> PublishAsync(long userId, CancellationToken ct)
    {
        ...
        return (bottle.Id, bottle.BottleNo);
    }
    ...
    public async Task<(Guid bottleId, string bottleNo, string content, int pickupCount)> PickupAsync(long userId, CancellationToken ct)
    {
        var now = _clock.UtcNow;
        var s = await _users.GetAsync(userId, ct);
        ...
        return (picked.Id, picked.BottleNo, picked.Content, count);
    }
